...
from cgcloud.toil.toil_box import ProtectLeader, ProtectBox
from cgcloud.toil.toil_box import ProtectWorker

log = logging.getLogger( __name__ )

leader = ProtectLeader.role( )
worker = ProtectWorker.role( )
node = ProtectBox.role( )

num_workers = 2


class ProtectClusterTests( MesosTestCase ):
    """
    Covers the creation of a Protect cluster from scratch and running a simple Toil job that invokes
    Docker on it.
    """
    cleanup = True
    create_image = True
    ...
    def setUpClass( cls ):
        ...
        super( ProtectClusterTests, cls ).setUpClass( )
        if cls.create_image:
    ...
    def tearDownClass( cls ):
        ...
        super( ProtectClusterTests, cls ).tearDownClass( )
